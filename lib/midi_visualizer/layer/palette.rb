...
module MIDIVisualizer
  module Layer
    # Palette
    #
    # The Palette implemnts a color lookup table that maps a value between 0 and
    # 1 either to a countinous or a discrete color palette.
    class Palette
      ...
      def to_a(n)
        Array.new(n) { |i| mix_ab(i.to_f / (n - 1)).freeze }
      end
      ...
      def mix_ab(r)
        Color::RGB.new(
          *@color_a.map.with_index { |e, i| e * (1 - r) + @color_b[i] * r },
          1.0
        )
      end
    end
